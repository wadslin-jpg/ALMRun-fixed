...
//	this->Hide();
	...
	// 先删除定时器，防止定时器回调访问已释放的 g_lua
	// Must delete timers before g_lua to prevent timer callbacks
	// from accessing the freed Lua state
	wxDELETE(g_timers);
...
void MerryFrame::OnShowEvent(wxShowEvent& e)
{
	...
	if (e.IsShown())
	{
		if (g_config && g_config->get(PlayPopupNotify))//是否播放提示音
...
#endif
		if (g_config && g_config->get(AutoPopup))
...
{
	// 拦截 Windows 关机/注销消息，同步执行清理，避免延迟析构导致的内存错误
	// Intercept session-end messages to perform synchronous cleanup,
	// preventing memory errors caused by deferred destruction during shutdown.
	if (message == WM_QUERYENDSESSION)
	{
		// 系统请求关机/注销，同步执行清理
		// System is requesting shutdown/logoff - clean up synchronously
	...
	{
		// 关机/注销已确认，确保清理完成
		// Shutdown/logoff confirmed - ensure cleanup is done
		if (!m_isClosing && g_lua)
			OnClose();
		return 0;
	}
